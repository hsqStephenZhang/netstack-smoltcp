...
2. 由于 iptables 和 IP 规则，握手数据包被定向到 loopback 设备并交给 TProxy 监听套接字 (TSocket) 。

3. TSocket 和 curl 发起的 TCP 连接完成握手。从 TSocket 看来，连接建立成功之后的四元组将是：
...
4. 我们（代理程序）获得真实目标服务器的地址为 `1.1.1.1:80`，连接此目标，将从 curl 接收到的连接称为 ingress stream， 直连到真实目标服务器的连接称为 egress stream。从代理程序的角度来看，这两个流将如下所示：

ingress stream: `127.0.0.1:45762` => `1.1.1.1:80`  (`127.0.0.1:45762` 属于 curl, `1.1.1.1:80` 属于 TSocket)
egress stream: `192.168.2.2:1111` => `1.1.1.1:80` (`192.168.2.2:1111` 属于 代理程序, `1.1.1.1:80` 属于远端真实服务器)

5. 调用 `tokio::io::copy_bidrectional` ，双向拷贝数据，完成简单的数据转发功能。

### TProxy 环路分析

当发生环路时，第 4 步会给我们带来麻烦：代理程序发起目标真实服务器的 tcp 连接时，握手数据包会被内核交给 TSocket/Tun 代理，成为新的 ingress stream，然后对于这个新的 ingress stream，则会重复前 3 个步骤，然后第 4 步的相同问题会再次发生，如此反复。
...
而每次 egress stream 建立后，在代理程序看来，就已经可以接着完成上述第5步的双向拷贝了（事实上也是如期进行的，只不过没有任何意义，这种拷贝没有尽头），因此，会有如下的**双向拷贝 - 连接对**:

-  ingress stream                    <=>  egress stream
...
正如你所看到的，从第二个流开始，每个 ingress stream 的本地地址（连接发起者的地址）都是前一个 egress stream 的本地地址。我们可以利用这一点，如果在 ingress stream 和 egress stream 中都**检测**到了相同的本地地址，即可判定为**环路**
